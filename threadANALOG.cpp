...
int contadorA = 1;
int contador_maxA = 31; //Cada 30 seg el thread vuelve a ready, con lo cual 1h = 120 veces 30 segundos
	...
	while (true) {
		...
		if(mode_change){
			...
			if(contadorA > contador_maxA){
				...
				//Poner a cero max_soil, min_soil, soil_average
				max_soil = 0.0;
				min_soil = 0.0;
				soil_average = 0.0;
				...
				//Poner a cero max_light, min light, light average
				max_light = 0.0;
				min_light = 0.0;
				light_average = 0.0;	
			}
